    ...
    const getPurchaseButton = useCallback(() =>
    {
        ...
        switch(purchaseState)
        {
            ...
            case CatalogPurchaseState.PURCHASE:
                return <button className="appearance-none border-0 w-full h-8 rounded-lg bg-black/10 text-black/40 text-xs font-medium" disabled><LayoutLoadingSpinnerView /></button>;
            ...
            default:
                return <button className="appearance-none border-0 w-full h-8 rounded-lg bg-primary text-primary-foreground text-xs font-medium hover:bg-primary/90 transition-colors" onClick={ () => setPurchaseState(CatalogPurchaseState.CONFIRM) }>{ LocalizeText('buy') }</button>;
        }
    ...
    return (
        <div className="flex flex-col h-full gap-2">
            <div className="flex flex-col gap-1.5 flex-1 min-h-0 overflow-auto">
                ...
                {
                    return (
                        <div key={ index } className={ `flex items-center gap-3 p-2.5 rounded-lg border cursor-pointer transition-colors ${ pendingOffer === offer ? 'border-black/20 bg-black/[0.04]' : 'border-black/[0.06] bg-black/[0.03] hover:border-black/[0.08]' }` } onClick={ () => setOffer(offer) }>
                            <i className="icon-hc-banner" />
                            <div className="flex flex-col items-end ml-auto">
                                <span className="text-sm text-black/85">{ getOfferText(offer) }</span>
                                <div className="flex items-center justify-end gap-1">
                                    { (offer.priceCredits > 0) &&
                                    <div className="flex items-center justify-end gap-1">
                                        <span className="text-xs text-black/50">{ offer.priceCredits }</span>
                                        <LayoutCurrencyIcon type={ -1 } />
                                    </div> }
                                    { (offer.priceActivityPoints > 0) &&
                                    <div className="flex items-center justify-end gap-1">
                                        <span className="text-xs text-black/50">{ offer.priceActivityPoints }</span>
                ...
                <div className="flex flex-col gap-2 p-2.5 bg-black/[0.03] rounded-lg border border-black/[0.06] shrink-0">
                    <div className="flex items-end">
                        <div className="flex flex-col flex-1">
                            <span className="text-sm font-medium text-black/85">{ getPurchaseHeader() }</span>
                            <span className="text-xs text-black/50">{ getPurchaseValidUntil() }</span>
                        </div>
                        <div className="flex flex-col gap-1">
                            { (pendingOffer.priceCredits > 0) &&
                                <div className="flex items-center justify-end gap-1">
                                    <span className="text-xs text-black/50">{ pendingOffer.priceCredits }</span>
                                    <LayoutCurrencyIcon type={ -1 } />
                                </div> }
                            { (pendingOffer.priceActivityPoints > 0) &&
                                <div className="flex items-center justify-end gap-1">
                                    <span className="text-xs text-black/50">{ pendingOffer.priceActivityPoints }</span>
