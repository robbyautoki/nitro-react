...
// ═══════════════════════════════════════════════════
// HELPER COMPONENTS — 1:1 from prototype
// ═══════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════
// LEVEL POPOVER — 1:1 from prototype
// ═══════════════════════════════════════════════════
      ...
      <PopoverTrigger asChild>
        <div className="flex items-center gap-1.5 px-2 py-1.5 rounded-lg cursor-pointer hover:bg-accent/50 transition-colors">
          ...
          <div className="w-[60px] h-[3px] bg-muted rounded-full overflow-hidden">
            <div className="h-full bg-gradient-to-r from-amber-400 to-yellow-500 rounded-full" style={{ width: `${(achievementScore % 100)}%` }} />
          </div>
        </div>
      </PopoverTrigger>
      <PopoverContent align="center" sideOffset={8} className="w-[260px] p-0">
        <div className="px-4 pt-3 pb-2 border-b border-border/50">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold">Level {Math.floor(achievementScore / 100) || 1}</span>
          </div>
        ...
        <div className="p-4 space-y-4">
          <div className="space-y-1.5">
            <Progress value={achievementScore % 100 || 67} className="h-2 [&>div]:bg-gradient-to-r [&>div]:from-amber-400 [&>div]:to-yellow-500" />
            <div className="flex items-center justify-between">
              <span className="text-[11px] text-muted-foreground tabular-nums">{achievementScore} XP</span>
              <span className="text-[11px] font-semibold tabular-nums">{achievementScore % 100 || 67}%</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl border border-amber-200 bg-amber-50">
            ...
            <div className="min-w-0">
              <p className="text-[10px] font-semibold text-amber-700">Nächste Belohnung</p>
...
// ═══════════════════════════════════════════════════
// TOP BAR — 1:1 from prototype TopBar
// ═══════════════════════════════════════════════════
...
export const PurseView: FC<{}> = props => {
  ...
  return (
    <TooltipProvider delayDuration={200}>
      {/* 1:1 prototype TopBar — floating centered */}
      <div className="fixed top-3 left-1/2 -translate-x-1/2 z-[69] pointer-events-auto inline-flex items-center gap-1 py-1.5 px-3 rounded-2xl bg-card/80 border border-border/40 shadow-lg backdrop-blur-md">
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <div className="flex items-center gap-1.5 px-2 py-1.5 rounded-lg cursor-pointer hover:bg-accent/50 transition-colors">
              <CurrencyIcon type="-1" />
              <span className="text-xs font-semibold tabular-nums">{creditsDisplay}</span>
            </div>
          </TooltipTrigger>
        ...
        {/* Level — 1:1 prototype LevelPopover */}
        <LevelPopover />
        ...
        {/* Tool Icons — 1:1 prototype TOOL_ICONS with CatalogIcon */}
