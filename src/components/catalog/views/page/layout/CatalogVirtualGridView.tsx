        ...
        <div className="flex flex-col h-full overflow-hidden">
            ...
            <div className="shrink-0 px-4 py-3 border-b border-black/[0.06]">
                <div className="flex items-center gap-2">
                    <Icon className="text-[11px] text-black/30" />
                    <span className="text-xs font-semibold text-black/60 uppercase tracking-[0.08em]">{ title }</span>
                    <span className="text-[10px] text-black/15">({ items.length })</span>
                </div>
            </div>
            ...
            <div className="flex-1 min-h-0 overflow-y-auto p-2">
                { items.length === 0 && (
                    <div className="flex items-center justify-center h-full text-black/15 text-xs">
                ...
                <div className="grid grid-cols-[repeat(auto-fill,68px)] gap-1">
                    { items.map((item, i) => (
                        <div
                            key={ i }
                            className="relative flex items-center justify-center rounded-lg border border-black/[0.06] bg-card cursor-pointer overflow-hidden transition-all duration-150 aspect-square group hover:border-indigo-500/60 hover:shadow-[0_0_10px_rgba(99,102,241,0.35)] hover:z-10"
                        ...
                        >
                            ...
                            <div className="absolute bottom-0 inset-x-0 flex flex-col items-center py-[2px] bg-white/80 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity z-10">
                                <span className="text-[7px] text-black/60 truncate max-w-[60px] leading-tight">{ item.name }</span>
                                { item.priceCredits > 0 &&
                                    <span className="text-[8px] font-bold text-amber-600">{ item.priceCredits }</span> }
                            </div>

                            {/* Count badge for frequent purchases */}
                            { item.count > 1 &&
                                <span className="absolute -top-0.5 -right-0.5 text-[9px] font-bold bg-black/[0.06] text-black/50 rounded-full min-w-[16px] h-[16px] flex items-center justify-center px-1 leading-none z-10">
                                    ×{ item.count }
                                </span> }
                        </div>
                    )) }
