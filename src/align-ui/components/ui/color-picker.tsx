...
const tokenCheckerboard = `
    repeating-conic-gradient(
        hsl(var(--align-bg-white-0)) 0 90deg,
        hsl(var(--align-bg-soft-200)) 0 180deg
    )
    0% -25%/6px 6px`;
...
function SliderTrack({ className, style, ...props }: AriaSliderTrackProps)
{
    return (
        <AriaSliderTrack
            ...
            style={ ({ defaultStyle }) => ({
                ...style,
                background: `${ defaultStyle.background }, ${ tokenCheckerboard }`,
            }) }
            { ...props }
        />
    );
...
function ColorThumb({ className, ...props }: AriaColorThumbProps)
{
    return (
        <AriaColorThumb
            className={ composeRenderProps(className, className => cn('z-50 size-3 rounded-full ring-2 ring-stroke-white-0', className)) }
            { ...props }
        />
    );
}
...
function ColorSwatchPickerItem({ className, ...props }: AriaColorSwatchPickerItemProps)
{
    return (
        <AriaColorSwatchPickerItem
            className={ composeRenderProps(className, className => cn('group/swatch-item cursor-pointer p-1 focus:outline-none', className)) }
            { ...props }
        />
    );
}
...
function ColorSwatch({ className, style, ...props }: AriaColorSwatchProps)
{
    return (
        <AriaColorSwatch
            className={ composeRenderProps(className, className => cn('size-4 rounded-full border-stroke-white-0 group-data-[selected=true]/swatch-item:border-2 group-data-[selected=true]/swatch-item:ring-[1.5px]', className)) }
            style={ ({ defaultStyle }) => ({
                ...style,
                background: `${ defaultStyle.background }, ${ tokenCheckerboard }`,
            }) }
...
const EyeDropperButton = React.forwardRef<HTMLButtonElement, React.HTMLAttributes<HTMLButtonElement>>((props, forwardedRef) =>
{
    const state = React.useContext(ColorPickerStateContext);

    const NativeEyeDropper = (globalThis as typeof globalThis & { EyeDropper?: new () => { open: () => Promise<{ sRGBHex: string }> } }).EyeDropper;

    if(!NativeEyeDropper || !state) return null;
